...
public class SendImageHelper {
	...
	public interface SnapCallback{
		void sendSnapImage(File file, String snapCoverName, String textContent, boolean isOrig);
	}
	...
	public static void sendImageAfterSelectCoverActivityResult(Intent data, SnapCallback callback) {
		final ArrayList<String> selectedImageFileList = data.getStringArrayListExtra(Extras.EXTRA_SCALED_IMAGE_LIST);
		final ArrayList<String> origSelectedImageFileList = data.getStringArrayListExtra(Extras.EXTRA_ORIG_IMAGE_LIST);
		final String snapCoverName = data.getStringExtra(Extras.EXTRA_COVER_NAME);
		final String contentText = data.getStringExtra(Extras.EXTRA_SNAP_TEXT);
		...
		for (int i = 0; i < selectedImageFileList.size(); i++) {
			...
			if (isOrig) {
				...
				if (callback != null) {
					callback.sendSnapImage(new File(origMD5Path), snapCoverName, contentText, isOrig);
				}
			} else {
				if (callback != null) {
					callback.sendSnapImage(imageFile, snapCoverName, contentText, isOrig);
				}
			}
		}
	}
	...
	public static void sendSnapCoverAfterSelfImagePicker(Context context, Intent data, final SnapCallback callback){
		boolean isOrig = data.getBooleanExtra(Extras.EXTRA_IS_ORIGINAL, false);
		final String coverName = data.getStringExtra(Extras.EXTRA_COVER_NAME);
		final String contentText = data.getStringExtra(Extras.EXTRA_SNAP_TEXT);
		...
		for (PhotoInfo photoInfo : photos) {
			new SendImageTask(context, isOrig, photoInfo, new Callback() {
				...
				public void sendImage(File file, boolean isOrig) {
					if (callback != null) {
						callback.sendSnapImage(file, coverName, contentText, isOrig);
					}
				}
			}).execute();
		}
